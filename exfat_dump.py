...
# tested with Python 2.7.9 (CONVERTED TO PYTHON 3)
...
def readMBR(f):
  ...
  for i in range(NUM_PARTITION):
    boot = mbr[offset+MBR_BOOT_OFFSET] # Byte value (int)
    type = mbr[offset+MBR_FSTYPE_OFFSET] # Byte value (int)
...
def displayMBR( pl ):
  ...
  for p in pl:
    ...
    # FIX: Remove ord() as 'type' and 'boot' are already integer byte values in Python 3
    if '{0}'.format(type) in partitionType:
      strType = partitionType[ '{0}'.format(type) ]
    else:
      strType = '??'
    print('    0x%02x 0x%02x %010d %010d %010d (%s)' % ( boot, type, start, start+size, size, strType ))
  print()
...
def readVBR( f, offset ):
  ...
  if unpack('>H', vbrData[SYNC_OFFSET:SYNC_OFFSET+2])[0]!=SYNC_VALUE:
    ...
    return None
  signature = vbrData[EXFAT_SIGN_OFFSET:EXFAT_SIGN_OFFSET+8].decode('ascii')
  if signature!='EXFAT   ':
  ...
  sectorSize = 1 << vbrData[EXFAT_SECTOR_SIZE_OFFSET] # Byte value is already an int in Py3
  vbr[ 'sectorSize' ] = sectorSize
  sectorsPerCluster = 1 << vbrData[EXFAT_CLUSTER_SIZE_OFFSET] # Byte value is already an int in Py3
...
def readClustersFat( f, vbr, cluster ):
  clusterData = b'' #read all clusters by following the chaining in FAT
...
def fsstat( partition, vbr ):
  ...
  # FIX: Multi-line print wrapped and used // for floor division
...
def unicode2ascii( name ):
  # In Python 3, all strings are Unicode (str). This function is designed to handle
  # Python 2 unicode strings where every other character is a NULL byte, or raw bytes from disk.
  if isinstance(name, bytes):
    # Treat as raw UTF-16LE bytes, decode, then encode back to ASCII (ignoring non-ASCII chars)
    # The double encode/decode simulates the Python 2 unicode to ASCII string conversion.
    return name.decode('utf-16le', errors='ignore').encode('ascii', errors='ignore').decode('ascii')
  elif isinstance(name, str):
    # If it's already a Python 3 string, assume it's good or convert to pure ASCII
...
def printDirRecord85( record ): #and 0x05
  type = record[0] # Already an int
  print('%02x:file dir entry, sc=%d' % ( type, record[1] ), end=' ') #sc = secondary count
  print(getDateTimeStr( unpack('<L', record[8:12])[0], record[20] ), end='') # record[20] is already an int
...
def printDirRecordC0( record ): #and 0x40
  type = record[0] # Already an int
  print('%02x:stream ext' % type, end=' ')
  #cluster, datalen, valid datalen, name len
  print('i=%d l=%d vl=%d nl=%d' % ( unpack('<L', record[20:24])[0],
    unpack('<Q', record[24:32])[0], unpack('<Q', record[8:16])[0], record[3] ), end=' ') # record[3] is already an int
  if record[1]&NOT_FAT_CHAIN_FLAG > 0: # record[1] is already an int
    print('nfc', end=' ')
  else:
    print('fat', end=' ')
...
def printDirRecordC1( record ): #and 0x41
  type = record[0] # Already an int
  print('%02x:filename ext=' % type, end=' ')
  # In Python 3, we must explicitly decode bytes read from the file
  print(str( record[2: EXFAT_DIRRECORD_SIZE], encoding='utf-16le', errors='ignore' ), end=' ')
...
def parseDir( f, vbr, clusterData ):
  ...
  remainingSC = 0
  while offset<len(clusterData) and clusterData[offset]!=0:
    type = clusterData[offset]
    if type==EXFAT_DIRRECORD_LABEL:
      count = clusterData[offset+1]
      label = str( clusterData[offset+2:offset+count*2], encoding='utf-16le', errors='ignore')
      if debugLevel>1:
      ...
      entry['secondaryCount'] = clusterData[offset+1]
      ...
      created10ms = clusterData[offset+20]
      modified10ms = clusterData[offset+21]
      ...
      if entryState == ENTRY_STATE_85_SEEN:
        ...
        entry['name']=''
        if clusterData[offset+1]&NOT_FAT_CHAIN_FLAG > 0: #no FAT chain
          entry['noFatChain'] = True
        else:
          entry['noFatChain'] = False

    elif (type&0x7f)==EXFAT_DIRRECORD_DEL_FILENAME_EXT: #or 0xc1
      # Note: Python 3 requires explicit decoding of byte slices read from the file
      filename = str( clusterData[offset+2: offset+EXFAT_DIRRECORD_SIZE], encoding='utf-16le', errors='ignore')
      if debugLevel>1: #only display debug
      ...
      if entryState == ENTRY_STATE_85_SEEN:
        if remainingSC >= 1: #concatenate only if remainingSC>0, IE if 0x85 and 0xC0 have been seen before
          ...
          if remainingSC==0: #last 0xC1
            entry['name'] = entry['name'][:entry['nameLen']] #truncate to real length (Py3 string length)
            if debugLevel>1:
...
def contentStat( f, vbr, globalList ):
  ...
  print('Directories: %d (%d Kb)' % (dir, dirContentSize//1024), end=' ')
...
if partOffset==0 and len(pl)>0:
  for p in pl: #if -o not used, will use the first non empty partition with type==7
    boot, type, start, size = p
    if type==PARTITION_TYPE_NTFS_EXFAT and size>0 and start>0: #must be of type==7, size>0 and starting sector>0
      partOffset = start
      foundPartition = True
      partition = p
      break
